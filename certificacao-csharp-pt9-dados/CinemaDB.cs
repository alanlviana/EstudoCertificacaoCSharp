...
namespace certificacao_csharp_pt9_dados
{
    public class CinemaDB
    {
        ...
        private async Task InserirRegistrosAsync()
        {
            ...
            using (Stream stream = assembly.GetManifestResourceStream("certificacao_csharp_pt9_dados.Diretores.txt"))
            ...
            using (Stream stream = assembly.GetManifestResourceStream("certificacao_csharp_pt9_dados.Filmes.txt"))
            using (StreamReader reader = new StreamReader(stream))
            {
                string line;
                while ((line = await reader.ReadLineAsync()) != null)
        ...
        string connectionString = $@"Data Source={databaseServer};Initial Catalog={banco};Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False";
